...
function baseball_formulation(players, old_lineups, num_overlap,stack_size, P,B1,B2,B3,C,SS,OF, players_teams, players_opp, players_games,players_stacks)
    ...
    @constraint(m, constr[i=1:num_games], used_game[i] <= sum(players_games[t, i]*players_lineup[t] for t in 1:num_players))
    ...
    @constraint(m, constr[i=1:num_teams], sum(players_teams[t, i]*(1-P[t])*players_lineup[t] for t in 1:num_players)<=5)
    

    #OVERLAP Constraint
    @constraint(m, constr[i=1:size(old_lineups)[2]], sum(old_lineups[j,i]*players_lineup[j] for  j in 1:num_players) <= num_overlap)
    ...
    @constraint(m, sum(used_stack_batters[i,j], i=1:num_teams for j in 1:num_stacks) >= 1)
